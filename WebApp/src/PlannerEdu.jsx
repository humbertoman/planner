...
// --- FIM: SEUS COMPONENTES INTERNOS ORIGINAIS (MANTIDOS) ---


// --- INÍCIO: COMPONENTE PRINCIPAL ---
const PlannerEdu = ({ user }) => { // <-- NOVO: Recebe o usuário logado
  
  // NOVO: State management com useState (substituindo o useStoredState)
  ...
  // --- NOVO: Carregando dados do Firestore em tempo real ---
   useEffect(() => {
      if (!user) return;
      ...
      return () => {
        unsubFolders(); unsubComponents(); unsubLessons(); unsubResources(); unsubEvaluations();
      };
    }, [user]);

  // --- NOVO: Funções CRUD adaptadas para o Firebase ---
  ...
  // --- MANTIDO: Lógica de UI (Cálculos, Filtros, etc.) ---
  ...
  const getFilteredComponents = () => {
    ...
    if (searchTerm) { filtered = filtered.filter(comp => comp.name.toLowerCase().includes(searchTerm.toLowerCase()) || comp.description.toLowerCase().includes(searchTerm.toLowerCase())); }
  ...
  const renderComponentCard = (component) => {
    ...
    return (
      <div key={component.id} onClick={() => setSelectedComponent(component)}
        className={`border border-gray-200 rounded-lg p-4 cursor-pointer transition-all ${selectedComponent?.id === component.id ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:shadow-md'} ${isCompactView ? 'mb-2' : 'mb-4'}`}>
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             {/* --- NOVO: Botão de Logout e email do usuário --- */}
          ...
          <nav className="p-4"><div className="space-y-2">
            {['components', 'lessons', 'resources', 'evaluations'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-colors ${activeTab === tab ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'text-gray-700 hover:bg-gray-50'}`}>
